...
export const createCustomFieldValue = async (req, res) => {
  try {
    const { ad_id, field_id, valeure } = req.body;
    const data = {
        ad_id: ad_id,
        field_id: field_id ,
        valeure: valeure,
...
export const updateCustomFieldValue = async (req, res) => {
  try {
    const { id } = req.params;
    const { ad_id, field_id, type } = req.body;
    const customFieldValue = await CustomFieldValue.findById(id);
    if (!customFieldValue) {
      return res.status(404).json({ message: "customFieldValue not found" });
    }
    if (name) customFieldValue.ad_id = ad_id;
    if (category) customFieldValue.field_id = field_id;
